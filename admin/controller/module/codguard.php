...
class Codguard extends \Opencart\System\Engine\Controller {
    ...
    public function index(): void {
        ...
        if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
            ...
            foreach ($this->request->post as $key => $value) {
                if (strpos($key, 'module_codguard_') === 0) {
                    $this->model_extension_codguard_module_codguard->saveSetting($key, $value);
                }
            }
        ...
        $saved_settings = $this->model_extension_codguard_module_codguard->loadAllSettings();
        ...
        $data['breadcrumbs'][] = array(
            ...
            'href' => $this->url->link('extension/codguard/module/codguard', 'user_token=' . $this->session->data['user_token'])
        );
        ...
        $data['action'] = $this->url->link('extension/codguard/module/codguard', 'user_token=' . $this->session->data['user_token']);
        ...
        // Get statistics
        $data['statistics'] = $this->model_extension_codguard_module_codguard->getStatistics();
        $data['recent_blocks'] = $this->model_extension_codguard_module_codguard->getRecentBlocks(10);
        ...
        $this->response->setOutput($this->load->view('extension/codguard/module/codguard', $data));
    }
    ...
    public function install(): void {
        ...
        $this->load->model('extension/codguard/module/codguard');
        $this->model_extension_codguard_module_codguard->install();

        // Add permissions to all user groups (OpenCart 4.x JSON format)
        $this->db->query("
            UPDATE `" . DB_PREFIX . "user_group`
            SET permission = JSON_ARRAY_APPEND(
                JSON_ARRAY_APPEND(permission, '$.access', 'extension/codguard/module/codguard'),
                '$.modify',
                'extension/codguard/module/codguard'
            )
        ");
    }
    ...
    public function uninstall(): void {
        // Load the model for database operations
        $this->load->model('extension/codguard/module/codguard');
        $this->model_extension_codguard_module_codguard->uninstall();
    ...
    protected function validate(): bool {
        if (!$this->user->hasPermission('modify', 'extension/codguard/module/codguard')) {
