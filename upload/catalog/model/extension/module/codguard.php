<?php
/**
 * CodGuard for OpenCart - Catalog Model
 *
 * @package    CodGuard
 * @author     CodGuard
...
class ModelExtensionModuleCodguard extends Model {
     ...
     */
    public function getCustomerRating($email) {
        ...
        // Log the request
        $this->log->write('CodGuard: Rating API called for ' . $email . ' - HTTP ' . $http_code);

        if ($curl_error) {
            $this->log->write('CodGuard: cURL error - ' . $curl_error);
            return null;
        }
        ...
        if ($http_code != 200) {
            $this->log->write('CodGuard: API returned status ' . $http_code);
            return null;
        }
        ...
        if (!isset($data['rating'])) {
            $this->log->write('CodGuard: Invalid API response - ' . $response);
            return null;
        }
     ...
     */
    public function logBlockEvent($email, $rating, $ip_address = '') {
        $this->db->query("
    ...
    public function queueOrder($order_id, $order_status_id) {
        ...
        if (!$order || empty($order['email'])) {
            $this->log->write('CodGuard: Order #' . $order_id . ' not found or has no email');
            return;
        }
        ...
        $this->log->write('CodGuard: Order #' . $order_id . ' queued for sync');

        // Schedule bundled send if not already scheduled
        $this->scheduleBundledSend();
    }
     ...
     */
    private function prepareOrderData($order, $order_status_id) {
        $shop_id = $this->config->get('module_codguard_shop_id');
        $refused_status = $this->config->get('module_codguard_refused_status');
     ...
     */
    private function scheduleBundledSend() {
     ...
     * Send orders to API
     */
    public function sendBundledOrders() {
        ...
        $this->log->write('CodGuard: Sending orders to API (immediate)');
